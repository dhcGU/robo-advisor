...
if __name__ == "__main__":
    ...
    while True:
        while True:
            ...
            if(len(symbol) < 1 or len(symbol) > 5 or hasNumber(symbol)):
                ...
                break
        symbol = symbol.upper()
        response = requests.get(compile_url("DAILY", symbol, API_key))
        weekly_response = requests.get(compile_url("WEEKLY", symbol, API_key))
    ...
    csv_name = os.path.join(os.path.dirname(__file__), "..", "data", symbol+"_prices.csv")
